...
web_ca.controller("CoordinateManager", function ($scope, $http) {
    ...
    //
    $http.post("http://" + site + "/getCoordinatesByWorldId/",document.getElementById("worldId").value)
        .success(function (data) {
            this.coordinate = data;
        }).error(function () {
            alert("GET COORDINATES: SERVER ERROR");
        });
    /*$http.post("http://" + site + "/get/",document.getElementById("worldId").value)
        .success(function (data) {
            this.coordinate = data;
        }).error(function () {
            alert("GET COORDINATES: SERVER ERROR");
...
function worldStates()
{
    ...
    var s=document.getElementById("worldState").innerHTML='<table class="table">';
    for(var i=0;i<counter;i++){
        s+="<tr><td><button style="+'"background-color:#'+arryColour[i].substring(2)+'"'+'class="btn btn-default btn-lg" ></button>';
